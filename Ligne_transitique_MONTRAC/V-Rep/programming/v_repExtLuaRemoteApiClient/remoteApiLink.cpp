			...
			while (foundACommand)
			{
				...
				for (unsigned int i=0;i<_commandsToHandle.size();i++)
				{
					if ( (_commandsToHandle[i]!=NULL)&&(_executionStatus[i][0]==0) )
					{
						...
						CLuaFunctionData* inOutData=_commandsToHandle[i];
...
void CRemoteApiLink::handleCommand(int cmdId,CLuaFunctionData* inOutData,bool blocking,bool threaded)
{
	// Insert the command:
	_lock();
	int index=-1;
...
void CRemoteApiLink::_handleCommand(int cmdId,CLuaFunctionData* inOutData)
{ // simxStart and simxFinish are handled elsewhere
	std::vector<CLuaFunctionDataItem>* inData=inOutData->getInDataPtr();
	if (cmdId==0)
	{ // simxAddStatusbarMessage
		int res=simxAddStatusbarMessage(_clientId,inData->at(1).stringData[0].c_str(),inData->at(2).intData[0]);
		inOutData->pushOutData(CLuaFunctionDataItem(res));
	}
	if (cmdId==1)
	...
	{ // simxAuxiliaryConsoleOpen
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(consoleHandle));
	}
	if (cmdId==3)
	...
	{ // simxGetObjectHandle
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(handle));
	}
	if (cmdId==10)
	{ // simxCloseScene
	...
	{ // simxCopyPasteObjects
		...
		if (res==0)
		{
			...
			inOutData->pushOutData(CLuaFunctionDataItem(nobj));
		}
	}
	if (cmdId==12)
	{ // simxLoadScene
	...
	{ // simxLoadModel
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(baseHandle));
	}
	if (cmdId==14)
	{ // simxLoadUI
		std::string fileAndPath(inData->at(1).stringData[0]);
		...
		if (res==0)
		{
			...
			inOutData->pushOutData(CLuaFunctionDataItem(handlesR));
		}
	}
	if (cmdId==15)
	...
	{ // simxGetBooleanParameter
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(p!=0));
	}
	if (cmdId==22)
	{ // simxGetFloatingParameter
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(p));
	}
	if (cmdId==23)
	...
	{ // simxGetStringParameter
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(std::string(p)));
	}
	if (cmdId==26)
	{ // simxSetFloatSignal
	...
	{ // simxGetFloatSignal
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(signalValue));
	}
	if (cmdId==30)
	...
	{ // simxGetStringSignal
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem((char*)signalValue,signalLength));
	}
	if (cmdId==32)
	{ // simxCreateDummy
		int dummyHandle;
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(dummyHandle));
	}
	if (cmdId==33)
	{ // simxDisplayDialog
		int dlgHandle;
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(dlgHandle));
			inOutData->pushOutData(CLuaFunctionDataItem(uiHandle));
		}
	}
	if (cmdId==34)
	{ // simxEndDialog
	...
	{ // simxGetDialogInput
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(std::string(input)));
	}
	if (cmdId==36)
	{ // simxGetDialogResult
		int result;
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(result));
	}
	if (cmdId==37)
	{ // simxEraseFile
	...
	{ // simxGetInMessageInfo
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res!=-1)
			inOutData->pushOutData(CLuaFunctionDataItem(info));
	}
	if (cmdId==42)
	...
	{ // simxGetJointForce
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(force));
	}
	if (cmdId==44)
	{ // simxGetJointMatrix
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(matrix));
	}
	if (cmdId==45)
	{ // simxGetJointPosition
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(position));
	}
	if (cmdId==46)
	{ // simxGetLastCmdTime
		int timInMs=simxGetLastCmdTime(_clientId);
		inOutData->pushOutData(CLuaFunctionDataItem(timInMs));
	}
	if (cmdId==47)
	{ // simxGetLastErrors
		int errorCnt;
		...
		if (res==0)
		{
			...
			inOutData->pushOutData(CLuaFunctionDataItem(errs));
		}
	}
	if (cmdId==48)
	{ // simxGetModelProperty
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(prop));
	}
	if (cmdId==49)
	{ // simxGetObjectChild
		int child;
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(child));
	}
	if (cmdId==50)
	{ // simxGetObjectFloatParameter
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(param));
	}
	if (cmdId==51)
	...
	{ // simxGetObjectGroupData
		...
		if (res==0)
		{
			...
			inOutData->pushOutData(CLuaFunctionDataItem(str));
		}
	}
	if (cmdId==53)
	{ // simxGetObjectOrientation
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(euler));
	}
	if (cmdId==54)
	{ // simxGetObjectPosition
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(pos));
	}
	if (cmdId==55)
	{ // simxGetObjectParent
		int parent;
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(parent));
	}
	if (cmdId==56)
	{ // simxGetObjects
		int handlesCount;
	...
	{ // simxGetObjectVelocity
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(lVel));
			inOutData->pushOutData(CLuaFunctionDataItem(aVel));
		}
	}
	if (cmdId==59)
	{ // simxGetPingTime
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(ptime));
	}
	if (cmdId==60)
	...
	{ // simxGetUIEventButton
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(butt));
			inOutData->pushOutData(CLuaFunctionDataItem(auxVals));
		}
	}
	if (cmdId==62)
	{ // simxGetUIHandle
	...
	{ // simxGetVisionSensorDepthBuffer
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(std::vector<float>(buffer,buffer+resolution[0]*resolution[1])));
			inOutData->pushOutData(CLuaFunctionDataItem(resolution));
		}
	}
	if (cmdId==65)
	{ // simxGetVisionSensorImage
		...
		if (res==0)
		{
			...
			if (inData->at(2).intData[0]&1)
				byteNb=1;
			inOutData->pushOutData(CLuaFunctionDataItem((char*)img,resolution[0]*resolution[1]*byteNb));
			inOutData->pushOutData(CLuaFunctionDataItem(resolution));
		}
	}
	if (cmdId==66)
	...
	{ // simxQuery
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem((char*)retSignalValue,retSignalLength));
	}
	if (cmdId==70)
	{ // simxReadCollision
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(collState!=0));
	}
	if (cmdId==71)
	{ // simxReadDistance
		float dist;
		...
		inOutData->pushOutData(CLuaFunctionDataItem(res));
		if (res==0)
			inOutData->pushOutData(CLuaFunctionDataItem(dist));
	}
	if (cmdId==72)
	{ // simxReadForceSensor
		unsigned char state;
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(state));
			if ((state&3)==1)
			{
				inOutData->pushOutData(CLuaFunctionDataItem(force));
				inOutData->pushOutData(CLuaFunctionDataItem(torque));
			}
		}
	}
	if (cmdId==73)
	{ // simxReadProximitySensor
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(state!=0));
			if (state!=0)
			{
				inOutData->pushOutData(CLuaFunctionDataItem(point));
				inOutData->pushOutData(CLuaFunctionDataItem(handle));
				inOutData->pushOutData(CLuaFunctionDataItem(normal));
			}
		}
	}
	if (cmdId==74)
	...
	{ // simxReadVisionSensor
		...
		if (res==0)
		{
			inOutData->pushOutData(CLuaFunctionDataItem(detectionState!=0));
			...
			for (int i=0;i<packets;i++)
			{
				...
				inOutData->pushOutData(CLuaFunctionDataItem(packet));
			}
		}
	}
	if (cmdId==77)
