...
public:
	...
	C4X4Matrix(const extIkReal m[4][4]);
	...
	void setIdentity();
	void buildInterpolation(const C4X4Matrix& fromThis,const C4X4Matrix& toThat,extIkReal t);
	...
	void rotateAroundX(extIkReal angle);
	void rotateAroundY(extIkReal angle);
	void rotateAroundZ(extIkReal angle);
	void translate(extIkReal x,extIkReal y,extIkReal z);

	C4X4Matrix operator* (const CMatrix& m) const;
	C4X4Matrix& operator= (const CMatrix& m);
	C4X4Matrix& operator= (const C4X4FullMatrix& m);

	inline void getInternalData(extIkReal d[12]) const
	{ // temporary routine, remove later
		X.getInternalData(d+0);
		M.getInternalData(d+3);
	}
	inline void setInternalData(const extIkReal d[12])
	{
	...
	inline void copyTo(extIkReal m[4][4]) const
	{
		for (int i=0;i<3;i++)
		{
			for (int j=0;j<3;j++)
	...
	inline void set(extIkReal m[4][4])
	{
		for (int i=0;i<3;i++)
		{
			for (int j=0;j<3;j++)
