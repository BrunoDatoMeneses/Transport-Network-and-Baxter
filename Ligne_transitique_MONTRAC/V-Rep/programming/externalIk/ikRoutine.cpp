...
void CIkRoutine::buildDeltaZRotation(C4X4FullMatrix& d0,C4X4FullMatrix& dp,extIkReal screwCoeff)
{
	d0.setIdentity();
	dp.clear();
	dp(0,1)=-1.0;
...
{	// rowJointIDs is NULL by default. If not null, it will contain the ids of the joints
	...
	CDummy* tooltip=Ct::ct->objCont->getDummy(ikElement->getTooltip());
	if (tooltip==NULL)
	{ // Should normally never happen!
		ikElement->setActive(false);
		return(NULL);
	}
	C3DObject* base=Ct::ct->objCont->getObject(ikElement->getBase());
	...
	{
		CDummy* alb=Ct::ct->objCont->getDummy(alternativeBaseForConstraints);
		if (alb!=NULL)
