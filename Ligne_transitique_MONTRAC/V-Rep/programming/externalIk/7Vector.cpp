...
C7Vector::C7Vector(const extIkReal m[4][4])
{
	set(m);
}
...
C7Vector::C7Vector(extIkReal angle,const C3Vector& pos,const C3Vector& dir)
...
void C7Vector::set(extIkReal m[4][4])
{
	C4X4Matrix tr(m);
...
void C7Vector::copyTo(extIkReal m[4][4]) const
...
void C7Vector::buildInterpolation(const C7Vector& fromThis,const C7Vector& toThat,extIkReal t)
