...
{	 
	...
	for(std::vector<XMLElement *>::const_iterator mit = meshes.begin(); mit != meshes.end(); mit++){
		...
		for(std::vector<XMLElement *>::const_iterator it = sources.begin(); it != sources.end(); it++) {
			...
			if (float_array != NULL) {
				if(XMLHelper::GetElementAttribute(float_array,"id",id)&&(float_array->FirstChild()!=NULL)) { // second part of condition added on 14/5/2014 (sometimes, geometries can have zero count of floats)
