...
void LUA_START_SERVER_CALLBACK(SLuaCallBack* p)
{
	p->outputArgCount=0;
	CLuaFunctionData D;
	int handle=-1; // means error
	std::string msg("Problem launching or communicating with the MTB server.");
	if (D.readDataFromLua(p,inArgs_START_SERVER,inArgs_START_SERVER[0],LUA_START_SERVER_COMMAND))
	{
		std::vector<CLuaFunctionDataItem>* inData=D.getInDataPtr();
		std::string mtbServerExecutableName(inData->at(0).stringData[0]);
		int portNumber=inData->at(1).intData[0];
		...
		std::string program(inData->at(2).stringData[0]);
			...
			QProcess::startDetached(cmd,strList,QString(currentDirAndPath.c_str()),NULL);
				...
				if(fork()==0)
				{
					execl(mtbServerExecutableName.c_str(),mtbServerExecutableName.c_str(),inData->at(0).stringData[0].c_str(),inData->at(1).stringData[0].c_str(),boost::lexical_cast<std::string>(inData->at(2).intData[0]).c_str(), (char*) 0);
					exit(0);
				}
				sleep(1);
			#endif
...
void LUA_STOP_SERVER_CALLBACK(SLuaCallBack* p)
{
	p->outputArgCount=0;
	CLuaFunctionData D;
	bool success=false;
	if (D.readDataFromLua(p,inArgs_STOP_SERVER,inArgs_STOP_SERVER[0],LUA_STOP_SERVER_COMMAND))
	{
		std::vector<CLuaFunctionDataItem>* inData=D.getInDataPtr();
...
void LUA_SET_INPUT_CALLBACK(SLuaCallBack* p)
{
	...
	if (D.readDataFromLua(p,inArgs_SET_INPUT,inArgs_SET_INPUT[0],LUA_SET_INPUT_COMMAND))
	{
		...
		{
			for (unsigned int i=0;i<4;i++)
				allMtbServers[index].robotInput[i]=(unsigned char)inData->at(1).intData[i];
			updateAllInputs();
			result=true;
		}
		else
...
VREP_DLLEXPORT unsigned char v_repStart(void* reservedPointer,int reservedInt)
{
	...
	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_START_SERVER,inArgs);
	simRegisterCustomLuaFunction(LUA_START_SERVER_COMMAND,strConCat("number mtbServerHandle,string message=",LUA_START_SERVER_COMMAND,"(string mtbServerExecutable,number portNumber,charBuffer program,table_4 jointPositions, table_2 velocities)"),&inArgs[0],LUA_START_SERVER_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_STOP_SERVER,inArgs);
	simRegisterCustomLuaFunction(LUA_STOP_SERVER_COMMAND,strConCat("boolean result=",LUA_STOP_SERVER_COMMAND,"(number mtbServerHandle)"),&inArgs[0],LUA_STOP_SERVER_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_STEP,inArgs);
	simRegisterCustomLuaFunction(LUA_STEP_COMMAND,strConCat("number result,string message=",LUA_STEP_COMMAND,"(number mtbServerHandle,number timeStep)"),&inArgs[0],LUA_STEP_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_GET_JOINTS,inArgs);
	simRegisterCustomLuaFunction(LUA_GET_JOINTS_COMMAND,strConCat("table_4 jointValues=",LUA_GET_JOINTS_COMMAND,"(number mtbServerHandle)"),&inArgs[0],LUA_GET_JOINTS_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_GET_OUTPUT,inArgs);
	simRegisterCustomLuaFunction(LUA_GET_OUTPUT_COMMAND,strConCat("table_4 outputValues=",LUA_GET_OUTPUT_COMMAND,"(number mtbServerHandle)"),&inArgs[0],LUA_GET_OUTPUT_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_GET_INPUT,inArgs);
	simRegisterCustomLuaFunction(LUA_GET_INPUT_COMMAND,strConCat("table_4 inputValues=",LUA_GET_INPUT_COMMAND,"(number mtbServerHandle)"),&inArgs[0],LUA_GET_INPUT_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_SET_INPUT,inArgs);
	simRegisterCustomLuaFunction(LUA_SET_INPUT_COMMAND,strConCat("boolean result=",LUA_SET_INPUT_COMMAND,"(number mtbServerHandle,table_4 inputValues)"),&inArgs[0],LUA_SET_INPUT_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_CONNECT_INPUT,inArgs);
	simRegisterCustomLuaFunction(LUA_CONNECT_INPUT_COMMAND,strConCat("boolean result=",LUA_CONNECT_INPUT_COMMAND,"(number inputMtbServerHandle,number inputBitNumber,number outputMtbServerHandle,number outputBitNumber,number connectionType)"),&inArgs[0],LUA_CONNECT_INPUT_CALLBACK);

	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_DISCONNECT_INPUT,inArgs);
	simRegisterCustomLuaFunction(LUA_DISCONNECT_INPUT_COMMAND,strConCat("boolean result=",LUA_DISCONNECT_INPUT_COMMAND,"(number inputMtbServerHandle,number inputBitNumber)"),&inArgs[0],LUA_DISCONNECT_INPUT_CALLBACK);
