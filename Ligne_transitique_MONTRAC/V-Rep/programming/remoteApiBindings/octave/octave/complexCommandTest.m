...
function complexCommandTest()
	...
	function retData=getCmdString(id,cnt,data)
		l=12+length(data);
		retData=simxPackInts([id cnt l]);
		retData=[retData data];
	end
	
	function replyData=waitForCmdReply(clID,vrp,cnt)
		while true
			[result string]=simxReadStringStream(clID,'repliesToRemoteApiClient',vrp.simx_opmode_streaming);
			if (result==vrp.simx_return_ok)
				while (length(string)~=0)
					headerPacked=string(1:12);
					header=simxUnpackInts(headerPacked);
					if (cnt==header(2))
						replyData='';
						if header(3)>12
	...
	if (clientID>-1)
		...
		simxWriteStringStream(clientID,'commandsFromRemoteApiClient',stringToSend,vrep.simx_opmode_oneshot);
		replyData=waitForCmdReply(clientID,vrep,cmdCnt) % display the reply from V-REP (in this case, just a string)
		...
		stringToSend=getCmdString(cmdID,cmdCnt,cmdData);
		simxWriteStringStream(clientID,'commandsFromRemoteApiClient',stringToSend,vrep.simx_opmode_oneshot);
		replyData=waitForCmdReply(clientID,vrep,cmdCnt);
		dummyHandle=simxUnpackInts(replyData) % display the reply from V-REP (in this case, the handle of the created dummy)
