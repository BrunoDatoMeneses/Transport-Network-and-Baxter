...
CVisionVelodyneHDL64E::CVisionVelodyneHDL64E(const int visionSensorHandles[4],float frequency,int options,float pointSize,float coloringDistances[2],float scalingFactor)
{
	for (int i=0;i<4;i++)
...
int CVisionVelodyneHDL64E::getVelodyneHandle()
{
	return(_velodyneHandle);
}

bool CVisionVelodyneHDL64E::areVisionSensorsExplicitelyHandled()
{
	for (int i=0;i<4;i++)
	{
...
bool CVisionVelodyneHDL64E::doAllObjectsExistAndAreVisionSensors()
{
	for (int i=0;i<4;i++)
	{
...
bool CVisionVelodyneHDL64E::handle(float dt,std::vector<float>& pts)
{
	pts.clear();
	bool retVal=true;
