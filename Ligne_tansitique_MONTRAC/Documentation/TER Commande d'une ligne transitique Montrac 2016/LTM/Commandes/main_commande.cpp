...
//creation et initialisation des objets Capteur et Actionneurs
	...
	ros::Rate loop_rate(20); //frequence de la boucle (25Hz)
...
//////////////////////////////////////////////
	...
	//A1 et A2
                ...
                if(M[32]){ if(PSx[1]) {M[32]--; M[1]++; M[33]++;}}					//t1

		if(M[1]>=1 && M[16]>=1){ if(DxD[1]) {M[1]--;M[16]--;M[2]++;} 				//t2
					else if(DxG[1]) { M[1]--;M[16]--;M[3]++;}}	 		//t3
		...
		if(M[13]>=1){ if(PSx[20]) {M[13]--;M[15]++;M[19]++;}}					//PS20

		if(M[19] >= 1 && M[34] >= 1) {if (DxD[11]){M[19]--;M[34]--;M[20]++;}      		//t16
					     else if(DxG[11]) { M[19]--;M[34]--;M[21]++;}}		//t17
		...
		if(M[26] && M[33] >=1){ if(1) {M[26]--;M[33]--;M[28]++;}}				//t24

		if(M[27] && M[33] >=1){ if(PSx[24]) {M[27]--;M[33]--;M[29]++;}}				//t25
		...
		if(M[32]>=1){ if(PSx[1]) {M[32]--;M[33]++;M[1]++;}} 					//PS1


                // ACTIONNEURS

		//Déverrouiller  les aiguillages 
                Dx[1]   = M[2]  || M[3] ;
		Dx[2]   = M[9]  || M[10];
		Dx[11]  = M[20] || M[21];
		Dx[12]  = M[28] || M[29];
		...
		//Verrouiller  les aiguillages 
		Vx[1]   = M[4]  || M[5] ;
		Vx[2]   = M[11] || M[12];
                Vx[11]  = M[22] || M[23];
                Vx[12]  = M[30] || M[31];

		//Aiguillages à droite 
                RxD[1]  = M[3]  ;
		RxD[2]  = M[10] ;
		RxD[11] = M[21] ;
		RxD[12] = M[29] ;

		//Aiguillages à gauche 
		RxG[1]  = M[2] ;
		RxG[2]  = M[9]  ;
		RxG[11] = M[20] ;
		RxG[12] = M[28] ;
