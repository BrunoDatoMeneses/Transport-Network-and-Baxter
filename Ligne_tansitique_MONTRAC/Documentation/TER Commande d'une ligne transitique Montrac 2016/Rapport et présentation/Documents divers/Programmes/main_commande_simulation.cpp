...
//creation et initialisation des objets Capteur, Aiguillage et point d'arret
...
//////////////////////////////////////////////
    ...
    // Positionnement des navettes devant les stop 12 et 24
	Deplacer_navettes(STx,RxD,RxG,Vx,Dx,PIx,&stopSt,&switchSt,12,24);

	while (ros::ok())
	{
		// Actualisation des capteurs
		Actualiser_Capteurs(PSx,DxD,DxG,CPx,CPIx,SensorSt);
		...
		if(M[53]>=1){ if(PSx[13]) {M[53]--; M[60]++;M[6]++;}}			//V

        //A7 et A8

        if(M[7]>=1 && M[60]>=1){	if(DxD[7]) {M[60]--;M[7]--;M[61]++;} 		//41
        ...
        if(M[80]&& M[11]>=1){	if(DxD[11]) {M[80]--;M[11]--;M[81]++;}		//t61
		...
		ros::spinOnce(); //Activation des fonction Callback si messages entrants
