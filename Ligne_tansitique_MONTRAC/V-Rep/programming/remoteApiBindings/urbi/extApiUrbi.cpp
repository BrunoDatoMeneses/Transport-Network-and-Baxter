...
{ 
    public: 
       ...
       vrep (const std::string &s):UObject(s) {
            ...
            UBindFunction(vrep ,simxGetCollectionHandle);
...
/*
        ...
        UList simxGetCollectionHandle (simxInt clientID,const simxChar* collectionName, simxInt operationMode) {
            UList result;
            simxInt handle;
            result.push_back(::simxGetCollectionHandle(clientID,collectionName, &handle, operationMode));
            result.push_back(handle);
            return result;
        }
