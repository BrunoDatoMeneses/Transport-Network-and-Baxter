...
const int inArgs_CREATE[]={
	9,
    sim_lua_arg_int|sim_lua_arg_table,2, // wheel motor handles
    sim_lua_arg_int|sim_lua_arg_table,2, // color sensor handles
    sim_lua_arg_int|sim_lua_arg_table,9, // IR sensor handles
    sim_lua_arg_int|sim_lua_arg_table,5, // US sensor handles
    sim_lua_arg_int|sim_lua_arg_table,6, // arm motor handles
    sim_lua_arg_int|sim_lua_arg_table,3, // finger motor handles
    sim_lua_arg_int|sim_lua_arg_table,2, // gripper distance sensor handles
    sim_lua_arg_int|sim_lua_arg_table,2, // gripper color sensor handles
    sim_lua_arg_int,0, // UI handle
};
...
void LUA_GETINFRARED_CALLBACK(SLuaCallBack* p)
{
	p->outputArgCount=0;
	CLuaFunctionData D;
	bool success=false;
	float distance=100.0f; // no detection
	if (D.readDataFromLua(p,inArgs_GETINFRARED,inArgs_GETINFRARED[0],LUA_GETINFRARED_COMMAND))
	{
		std::vector<CLuaFunctionDataItem>* inData=D.getInDataPtr();
...
void LUA_GETULTRASONIC_CALLBACK(SLuaCallBack* p)
{
	p->outputArgCount=0;
	CLuaFunctionData D;
	bool success=false;
	float distance=100.0f; // no detection
	if (D.readDataFromLua(p,inArgs_GETULTRASONIC,inArgs_GETULTRASONIC[0],LUA_GETULTRASONIC_COMMAND))
	{
		std::vector<CLuaFunctionDataItem>* inData=D.getInDataPtr();
		int k3Index=getK3IndexFromHandle(inData->at(0).intData[0]);
		int sensorIndex=inData->at(1).intData[0];
