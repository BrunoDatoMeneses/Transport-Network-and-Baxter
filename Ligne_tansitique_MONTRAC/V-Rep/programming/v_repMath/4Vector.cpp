...
void C4Vector::buildInterpolation_otherWayRound(const C4Vector& fromThis,const C4Vector& toThat,float t)
{
	...
	(*this)=(AA*C4Vector((piValTimes2-r(0))*t,C3Vector(r(1)*-1.0f,r(2)*-1.0f,r(3)*-1.0f)));
	// Already normalized through * operator
}
...
void C4Vector::buildRandomOrientation()
{
	...
	data[0]=sqrtf(1.0f-u(0))*sin(piValTimes2*u(1));
	data[1]=sqrtf(1.0f-u(0))*cos(piValTimes2*u(1));
	data[2]=sqrtf(u(0))*sin(piValTimes2*u(2));
	data[3]=sqrtf(u(0))*cos(piValTimes2*u(2));
}
