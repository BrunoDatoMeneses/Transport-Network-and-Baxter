...
C4X4Matrix::C4X4Matrix(const extIkReal m[4][4])
{
	for (int i=0;i<3;i++)
	{
		for (int j=0;j<3;j++)
...
void C4X4Matrix::buildInterpolation(const C4X4Matrix& fromThis,const C4X4Matrix& toThat,extIkReal t)
...
void C4X4Matrix::rotateAroundX(extIkReal angle)
{
	C4X4Matrix rot;
	rot.setIdentity();
...
void C4X4Matrix::rotateAroundZ(extIkReal angle)
{
	C4X4Matrix rot;
	rot.setIdentity();
...
void C4X4Matrix::buildXRotation(extIkReal angle)
{
	setIdentity();
	M.buildXRotation(angle);
}

void C4X4Matrix::buildYRotation(extIkReal angle)
{
	setIdentity();
	M.buildYRotation(angle);
}

void C4X4Matrix::buildZRotation(extIkReal angle)
{
	setIdentity();
	M.buildZRotation(angle);
}

void C4X4Matrix::buildTranslation(extIkReal x,extIkReal y,extIkReal z)
{
	setIdentity();
	X(0)=x;
	X(1)=y;
...
void C4X4Matrix::translate(extIkReal x,extIkReal y,extIkReal z)
{
	X(0)+=x;
	X(1)+=y;
	X(2)+=z;
