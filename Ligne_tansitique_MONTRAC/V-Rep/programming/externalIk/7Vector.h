...
public:
	...
	C7Vector(const extIkReal m[4][4]);
	C7Vector(const C4X4Matrix& m);
	C7Vector(extIkReal angle,const C3Vector& pos,const C3Vector& dir);
	~C7Vector();

	void setIdentity();
	void set(extIkReal m[4][4]);
	...
	void buildInterpolation(const C7Vector& fromThis,const C7Vector& toThat,extIkReal t);
	void inverse();
	void copyTo(extIkReal m[4][4]) const;
	...
	inline void getInternalData(extIkReal d[7]) const
	{
		X.getInternalData(d+0);
		Q.getInternalData(d+3);
	}
	inline void setInternalData(const extIkReal d[7])
	{
