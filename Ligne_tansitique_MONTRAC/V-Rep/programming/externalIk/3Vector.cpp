...
C3Vector::C3Vector(extIkReal v0,extIkReal v1,extIkReal v2)
{
	data[0]=v0;
	data[1]=v1;
	data[2]=v2;
...
C3Vector::C3Vector(const extIkReal v[3])
{
	data[0]=v[0];
	data[1]=v[1];
	data[2]=v[2];
...
extIkReal C3Vector::getAngle(const C3Vector& v) const
...
void C3Vector::buildInterpolation(const C3Vector& fromThis,const C3Vector& toThat,extIkReal t)
{
	(*this)=fromThis+((toThat-fromThis)*t);
}

const C3Vector C3Vector::oneOneOneVector(1.0,1.0,1.0);
