...
const int inArgs_COMPUTE[]={
	2,
	sim_lua_arg_float|sim_lua_arg_table,0,
	sim_lua_arg_bool,0,
};
	...
	if (D.readDataFromLua(p,inArgs_COMPUTE,inArgs_COMPUTE[0],LUA_COMPUTE_COMMAND))
	{
		...
		else
		{
			...
			if (result)
			{
				...
				D.pushOutData(CLuaFunctionDataItem(verticesOut));
				if (generateIndices)
					D.pushOutData(CLuaFunctionDataItem(indicesOut));
			}
			else
...
VREP_DLLEXPORT unsigned char v_repStart(void* reservedPointer,int reservedInt)
{
	...
	simRegisterCustomLuaFunction(LUA_COMPUTE_COMMAND,strConCat("table vertices[,table indices]=",LUA_COMPUTE_COMMAND,"(table vertices,bool generateIndices)"),&inArgs[0],LUA_COMPUTE_CALLBACK);
